...
interface DatabaseDao {
    ...
    @Query("SELECT thumbnailUrl FROM song WHERE liked AND thumbnailUrl IS NOT NULL ORDER BY likedDate DESC LIMIT 4")
    fun likedSongsThumbnails(): Flow<List<String>>
    ...
    @Query("""
        ...
        JOIN (SELECT songId FROM event GROUP BY songId ORDER BY SUM(playTime) DESC LIMIT 4)
        ON song.id = songId
        WHERE song.thumbnailUrl IS NOT NULL
    """)
    fun mostPlayedSongsThumbnails(): Flow<List<String>>
    ...
    fun addSongsToPlaylist(
        ...
        if (prepend) {
            ...
            songsToInsert.forEach { (id, setVideoId) ->
                val existingSong = getSongByIdBlocking(id)!!
                if (setInLibrary && existingSong.song.inLibrary == null) {
                    inLibrary(id, now)
                }
                insert(
                    PlaylistSongMap(
    ...
    fun insert(
        ...
        mediaMetadata.artists.forEachIndexed { index, artist ->
            ...
            // If artist already existed (insert was ignored), fill in thumbnail if missing
            mediaMetadata.thumbnailUrl?.let { thumb ->
                updateArtistThumbnailIfMissing(artistId, thumb)
            }
