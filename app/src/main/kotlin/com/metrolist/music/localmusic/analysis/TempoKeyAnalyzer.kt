...
constructor() {
    fun estimateBpm(samples: FloatArray, sampleRate: Int): Float {
        ...
        for (index in onset.indices) {
            onset[index] = max(0f, energies[index + 1] - energies[index])
        }
        ...
        for (bpm in MIN_BPM..MAX_BPM) {
            ...
            val halfLag = lag / 2
            if (halfLag > 0) {
                for (index in halfLag until onset.size) {
                    score += onset[index] * onset[index - halfLag] * 0.35f
                }
            }
            if (score > bestScore) {
                bestScore = score
        ...
        return if (bestBpm < 80f && bestBpm * 2f <= MAX_BPM) {
            bestBpm * 2f
        } else {
            bestBpm
        }
    ...
    fun estimateKey(samples: FloatArray, sampleRate: Int): String {
        ...
        return if (major[majorIndex] > minor[minorIndex]) {
            KEY_NAMES[majorIndex]
        } else {
            "${KEY_NAMES[minorIndex]}m"
        }
    }
    ...
    private fun frameEnergies(samples: FloatArray): FloatArray {
        ...
        return FloatArray(count) { frameIndex ->
            ...
            var sum = 0f
            for (offset in 0 until ENERGY_FRAME_SIZE) {
                val sample = samples.getOrNull(start + offset) ?: 0f
                sum += abs(sample)
            }
    ...
    private fun pitchClassEnergy(samples: FloatArray, sampleRate: Int): FloatArray {
        ...
        while (start + KEY_FRAME_SIZE <= samples.size) {
            frame.fill(0f)
            for (offset in 0 until KEY_FRAME_SIZE) {
                frame[offset] = samples[start + offset] * window[offset]
            }
            val power = AudioMath.fftPower(frame, KEY_FFT_SIZE)
