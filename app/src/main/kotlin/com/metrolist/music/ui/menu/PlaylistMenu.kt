    ...
    LazyColumn(
        ...
        item {
            val gridActions = buildList {
                ...
                add(
                    NewAction(
                        ...
                        onClick = {
                            onDismiss()
                            val shareText = if (isChinaPlaylist) {
                                val browseId = dbPlaylist?.playlist?.browseId ?: ""
                ...
                if (isChinaPlaylist) {
                    add(
                        NewAction(
                            ...
                            onClick = {
                                onDismiss()
                                val browseId = dbPlaylist?.playlist?.browseId ?: return@NewAction
                                val parts = browseId.removePrefix("china_").split("_", limit = 2)
                                if (parts.size < 2) return@NewAction
                                val sourceId = parts[0]
                                val songlistId = parts[1]
                                val source = com.metrolist.chinamusic.model.MusicSource.fromId(sourceId)
                                    ?: com.metrolist.chinamusic.model.MusicSource.NETEASE
                                val currentPlaylist = dbPlaylist ?: return@NewAction
                                coroutineScope.launch(Dispatchers.IO) {
                                    com.metrolist.chinamusic.ChinaMusicApi.getSonglistDetail(
                                        id = songlistId, page = 1, limit = 10000, source = source,
                                    ).onSuccess { detail ->
                                        ...
                                        database.transaction {
                                            ...
                                            addSongsToPlaylist(currentPlaylist, songIds, setInLibrary = false)
                                        }
                                        withContext(Dispatchers.Main) {
                                            Toast.makeText(context, "歌单已更新，共${detail.songs.size}首", Toast.LENGTH_SHORT).show()
                                        }
                                    }.onFailure {
                                        withContext(Dispatchers.Main) {
            ...
            NewActionGrid(
                ...
                columns = gridActions.size.coerceIn(2, 3),
            )
        }
            ...
            Material3MenuGroup(
                items =
                    buildList {
                        ...
                        if (autoPlaylist != true && !isGuest) {
                            add(
                                Material3MenuItemData(
                                    ...
                                    description = { Text(text = stringResource(R.string.delete_playlist_desc)) },
                                    icon = {
                                        Icon(
                        ...
                        val chinaShareLink = if (isChinaPlaylist) {
                            dbPlaylist?.playlist?.browseId?.let {
                                com.metrolist.chinamusic.PlaylistUrlParser.getPlaylistUrlFromBrowseId(it)
                            }
                        } else null
                        val effectiveShareLink = chinaShareLink ?: playlist.playlist.shareLink
                        effectiveShareLink?.let { shareLink ->
                            add(
                                Material3MenuItemData(
                                    title = { Text(text = stringResource(R.string.share)) },
                                    description = { Text(text = stringResource(R.string.share_playlist_desc)) },
                                    icon = {
                                        Icon(
                                            painter = painterResource(R.drawable.share),
                                            contentDescription = null,
