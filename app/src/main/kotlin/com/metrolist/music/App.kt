...
class App :
    ...
    private suspend fun cleanupYouTubeSyncData() {
        val settings = dataStore.data.first()
        if (settings[YouTubeSyncCleanedUpKey] == true) {
            // v2: also clean China songs affected by addSongsToPlaylist bug
            val v2Key = "youtube_sync_cleaned_up_v2"
            val v2Done = settings[androidx.datastore.preferences.core.booleanPreferencesKey(v2Key)]
            if (v2Done == true) return
            withContext(Dispatchers.IO) {
                database.query { clearYouTubeSyncedLibrary() }
                dataStore.edit { it[androidx.datastore.preferences.core.booleanPreferencesKey(v2Key)] = true }
                Timber.d("YouTube sync library data cleaned up (v2 - including China songs)")
            }
            return
        }
        withContext(Dispatchers.IO) {
            database.query {
                clearYouTubeSyncedLibrary()
            }
            dataStore.edit { it[YouTubeSyncCleanedUpKey] = true }
            Timber.d("YouTube sync library data cleaned up")
        }
    }
