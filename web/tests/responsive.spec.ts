...
test('music libraries are visible in settings and filter the NAS catalog', async ({ page }) => {
  ...
  await expect(page.getByRole('textbox', { name: '随身硬盘 的名称' })).toHaveCSS('font-size', '16px')
  await expect(page.getByRole('textbox', { name: '随身硬盘 的名称' })).toHaveCSS('height', '48px')
  await expect(page.getByRole('combobox', { name: '随身硬盘 的设备类型' })).toHaveCSS('font-size', '16px')
  await expect(page.getByRole('combobox', { name: '随身硬盘 的设备类型' })).toHaveCSS('height', '48px')
  await expect(page.getByRole('textbox', { name: '名称', exact: true })).toHaveCSS('font-size', '16px')
  await expect(page.getByRole('textbox', { name: '名称', exact: true })).toHaveCSS('height', '48px')
  await expect(page.getByRole('combobox', { name: '设备类型', exact: true })).toHaveCSS('font-size', '16px')
  await expect(page.getByRole('combobox', { name: '设备类型', exact: true })).toHaveCSS('height', '48px')

  await page.getByRole('button', { name: '返回设置' }).click()
  await page.getByRole('button', { name: /音乐源/ }).click()
  await expect(page.getByRole('heading', { name: '音乐源' })).toBeVisible()
  await expect(page.getByRole('textbox', { name: '名称' })).toHaveCSS('font-size', '16px')
  await expect(page.getByRole('textbox', { name: '名称' })).toHaveCSS('height', '48px')
  await expect(page.getByRole('textbox', { name: 'API 地址' })).toHaveCSS('font-size', '16px')
  await expect(page.getByRole('textbox', { name: 'API 地址' })).toHaveCSS('height', '48px')
  await expect(page.getByLabel('API 密钥')).toHaveCSS('font-size', '16px')
  await expect(page.getByLabel('API 密钥')).toHaveCSS('height', '48px')
