...
export function TrackList({ tracks, favorites, onPlay, onFavorite, trailingAction, selectedIds, onSelection, sort, onSort, virtualized = false }: { tracks: Track[]; favorites: Track[]; onPlay: (track: Track, queue: Track[]) => void; onFavorite?: (track: Track) => void; trailingAction?: (track: Track) => ReactNode; selectedIds?: string[]; onSelection?: (ids: string[]) => void; sort?: LibrarySort; onSort?: (value: LibrarySort) => void; virtualized?: boolean }) {
  ...
  const openMenu = (track: Track, index: number, mode: 'sheet' | 'context', x: number, y: number, opener: HTMLElement) => {
    ...
    const top = mode === 'context' ? Math.min(Math.max(12, y), Math.max(12, window.innerHeight - 260)) : 0
